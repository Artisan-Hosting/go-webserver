...
type captchaVerifier func(endpoint, secret, token string) (bool, error)
type mailDelivery func(EmailPayload) error
...
func contactHandler(w http.ResponseWriter, r *http.Request) {
	contactHandlerWithDependencies(simError, verifyCaptcha, sendMail)(w, r)
}

func contactHandlerWithDependencies(simulatedError string, verify captchaVerifier, deliver mailDelivery) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if endpoint, secret, enabled := captchaConfig(); enabled {
			...
			ok, err := verify(endpoint, secret, token)
			if err != nil {
		...
		if err := deliver(EmailPayload{
			...
			log.Printf("owner email delivery failed: %v", err)
		}
